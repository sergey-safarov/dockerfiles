...
try:
    ...
    logger.error("Please run with Python 3.7 or higher")
    exit(1)

try:
    from kubernetes import client, config
except:
    logger.error("Please install kubernetes - `pip3 install kubernetes`")
    exit(1)

try:
    import boto3
except:
    logger.error("Please install boto3 - `pip3 install boto3`")
    exit(1)

try:
    import awscli.__main__ as _
except:
    logger.error("Please install aws-cli - `pip3 install awscli`")
    exit(1)
...
try:
    ...
    logger.error("please run within cluster pod")
    # exit(1)
...
def handle_messages(operations_log: List[str], message: str) -> List[str]:
    logger.info(message)
    operations_log = handle_messages(operations_log, message)
...
try:
    ...
    logger.error("Please setup AWS credentials")
    exit(1)
...
def get_statful_set_by_ip(ip: str) -> str:
    ...
    return [
        i.name for i in pod_by_ip.metadata.owner_references if i.kind == "StatefulSet"
    ][0]
...
def prepare_address_objects(pool: List[str]) -> (List[Dict], List[str]):
    ...
    inaccessible_addresses = [
        i for i in pool if i not in [a["PublicIp"] for a in addresses]
    ]

    return addresses, inaccessible_addresses


def assign_address_to_instance(
    addresses: List[Dict],
    instance: str,
    interface: str,
    my_ip: str,
...
def get_pods_by_instance_ip(elastic_ip_obj: Dict) -> List[V1Pod]:
    ...
    return [
        ...
        if i.status.pod_ip == elastic_ip_obj["PrivateIpAddress"]
    ]
...
def execute_ssm_command(
    instance: str, command: str, instance_id: str
) -> (List[str], str):
...
def fix_ip_routes(rules: List[str], my_ip: str, instance_id: str, operations_log: List[str]) -> List[str]:
    ...
    if lookup_pod_ip_lines:
        ...
        operations_log = handle_messages(operations_log, f"First number - {first_number}")
        ...
        operations_log = handle_messages(operations_log, f"Delta numbers: {delta_numbers}")

        for number in delta_numbers:
            out, err = execute_ssm_command(
                instance_id,
                f"ip rule del from {my_ip} to default table {number}",
                instance_id,
            )
            operations_log = handle_messages(operations_log, out)
            if err:
                raise Exception(err)
        out, err = execute_ssm_command(
            instance_id,
            f"ip rule add from {my_ip} to default table {first_number}",
            instance_id,
        )
        operations_log = handle_messages(operations_log, out)
        if err:
...
def main():
    ...
    operations_log = handle_messages(operations_log, f"My IP: {my_ip}")
    ...
    operations_log = handle_messages(operations_log, f"Stateful IPs: {stateful_ips}")

    if my_ip not in stateful_ips:
        return jsonify(
            {
                "error": f"error: current pod is not member of {parent_stateful_set} statefulset",
                "operations_log": operations_log,
            }
        )

    host_ip = get_host_ip_by_pod_ip(my_ip)
    operations_log = handle_messages(operations_log, f"Host IP: {host_ip}")

    if not host_ip:
        return jsonify(
            {
                "error": "error: cannot determine host IP",
                "operations_log": operations_log,
            }
        )
    ...
    operations_log = handle_messages(operations_log, 
    ...
    if not instance_id:
        return jsonify(
            {
                "error": "error: cannot determine EC2 Instance-ID",
                "operations_log": operations_log,
            }
        )
    ...
    if network_interface_host != network_interface_pod:
        ...
        ip_rules_for_log = [
            i for i in ip_rules if f"from {my_ip} to " in i and "lookup main" not in i
        ]
        if ip_rules_for_log:
            operations_log = handle_messages(operations_log, f"IP rules: {ip_rules_for_log[0]}")
        else:
            operations_log = handle_messages(operations_log, f"Error: no ip rules found matching the pattern")
        if err:
            operations_log = handle_messages(operations_log, err)

        try:
            operations_log = fix_ip_routes(ip_rules, my_ip, instance_id, operations_log)
        except Exception as e:
            operations_log = handle_messages(operations_log, f"Failed to fix ip routes - {e}")

    public_ip_pool = get_statefulset_public_ip_pool(NAMESPACE, parent_stateful_set)
    operations_log = handle_messages(operations_log, f"Public IP pool: {public_ip_pool}")
    if not public_ip_pool:
        return jsonify(
            {
                "error": f"error: cannot determine public IP addresses for service: {parent_stateful_set}",
                "operations_log": operations_log,
            }
        )

    operations_log = handle_messages(operations_log, 
        f"EC2 host with ID {instance_id} have secondary IP address: {my_ip}."
    )
    operations_log = handle_messages(operations_log, 
        f"Please map with {my_ip} IP address one of this elastic IP addresses:"
    )
    operations_log = handle_messages(operations_log, f"{public_ip_pool}")

    address_objects, inaccessibles = prepare_address_objects(public_ip_pool)

    operations_log = handle_messages(operations_log, f"Inaccessibles IP addresses: {inaccessibles}")
    ...
    for ip in address_objects:
        ...
        if ip.get("PrivateIpAddress") and ip.get("InstanceId"):
            ...
            if not pods:
                operations_log = handle_messages(operations_log, f"No pods found associated with address {ip}")
                release_address(ip)
                ...
                if not nodes:
                    return jsonify(
                        {
                            "error": f"No nodes found for IP address {ip}",
                            "operations_log": operations_log,
                        }
                    )
                ...
                for node in nodes:
                    ...
                    ] != [pods[0].status.host_ip]:
                        operations_log = handle_messages(operations_log, 
                            f"No pods found associated with address {ip}"
                        )
                        release_address(ip)
                        continue

                if pods[0].metadata.name not in [
                    i.metadata.name for i in stateful_pods
                ]:
                    operations_log = handle_messages(operations_log, 
                        f"{pods[0].metadata.name} associated with address {ip} and not a part of stateful set"
                    )
                    release_address(ip)
    ...
    if not mapped_ip:
        mapped_ip, errors = assign_address_to_instance(
            address_objects, instance_id, network_interface_host, my_ip
        )

    if not mapped_ip and errors:
        return jsonify(
            {
                "error": f"Failed to assign elastic IP - {errors}",
                "operations_log": operations_log,
            }
        )
    ...
    operations_log = handle_messages(operations_log, f"Successfully assigned IP - {mapped_ip}")
    ...
    res, err = execute_ssm_command(
        instance_id, f"ipset list {parent_stateful_set}", instance_id
    )

    operations_log = handle_messages(operations_log, f"IP set list - {res}")
    ...
    if any(["The set with the given name does not exist" in i for i in err]):
        ...
        operations_log = handle_messages(operations_log, f"creating ipset - {command}")
        if err:
            operations_log = handle_messages(operations_log, f"IP set create errors - {err}")
    ...
    operations_log = handle_messages(operations_log, f"Stateful set addresses - {statful_set_addresses}")

    ipset_results, err = execute_ssm_command(
        instance_id, f"ipset list {parent_stateful_set}", instance_id
    )
    operations_log = handle_messages(operations_log, f"IP set list - {ipset_results}")
    ...
    operations_log = handle_messages(operations_log, f"IPset addresses - {ipset_addresses}")

    for ip in ipset_addresses:
        if ip not in statful_set_addresses:
            res, err = execute_ssm_command(
                instance_id, f"ipset del {parent_stateful_set} {ip}", instance_id
            )
            operations_log = handle_messages(operations_log, f"IP set del res - {res}")

    for ip in statful_set_addresses:
        res, err = execute_ssm_command(
            instance_id, f"ipset add {parent_stateful_set} {ip}", instance_id
        )
        operations_log = handle_messages(operations_log, f"IP set add res - {res}")
    ...
    if err:
        operations_log = handle_messages(operations_log, f"iptables resolve errors - {err}")
    ...
    for r in res:
        if (
            ...
            operations_log = handle_messages(operations_log, f"Found matching iptables rule")

    if not iptable_res_found:
        operations_log = handle_messages(operations_log, f"iptables - {res}")
        ...
        res, err = execute_ssm_command(instance_id, command, instance_id)
        if err:
            operations_log = handle_messages(operations_log, f"iptables add errors - {err}")
        else:
            operations_log = handle_messages(operations_log, f"added iptables rule - {command}")
